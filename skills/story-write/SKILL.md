...
4. **撰写/重写小节文件**，写入 `~/.storytelling/biography/<第N部-部名>/<年份>/[<月份MM>/]<NN>-<小节标题>.md`：
   - 目录结构见 STORY.md「五部结构」：部（人生阶段）→ 年 → 可选月份层。第五部等高密度
     部分保留 `<年份>/<月份MM>/`；前四部年份目录下直接放小节文件，不设月份目录。相邻两部
     共享边界年（毕业的夏天为界），按事件季节归部。
   - 目录不存在就创建；`<NN>` 为章内（无月份层时为年份内）两位序号，新开小节取已有小节数 +1。
